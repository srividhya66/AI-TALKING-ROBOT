...
def main():
    ...
    engine.say("Hey sri vidhya, I am your robot. How can I assist you today?")
    engine.runAndWait()
    ...
    while True:
        ...
        if "thank you" in user_input.lower():
            engine.say("SRI ELA UNNAV!")
            engine.runAndWait()
            continue
        if "who is abdul kalam" in user_input.lower():
            engine.say("Abdul Kalam: People's president, extraordinary Indian BBC. Archived from the original on 30 July 2015. Retrieved 28 June 2015.Ramzan & Rameswaram: His ties with the island – The Times of India The Times of India. 30 July 2015. Archived from the original on 23 August 2015. Retrieved 4 August 2015.")
            engine.runAndWait()
            continue
        if "what is machine learning" in user_input.lower():
            engine.say("Machine learning is a method of data analysis that automates analytical model building. It is a branch of artificial intelligence based on the idea that systems can learn from data, identify patterns and make decisions with minimal human intervention.")
            engine.runAndWait()
            continue
        if "good afternoon" in user_input.lower():
            ...
            engine.runAndWait()
            continue
        if "what is cyber security" in user_input.lower():
            engine.say("Cyber security is the practice of defending computers, servers, mobile devices, electronic systems, networks, and data from malicious attacks. It's also known as information technology security or electronic information security")
            engine.runAndWait()
            continue
